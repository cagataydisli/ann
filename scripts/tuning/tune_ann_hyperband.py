...
def main():
    ...
    tuner = kt.Hyperband(
        ...
        max_epochs=30,
        factor=3,
        directory=str(PROCESSED_DIR / "kt_hyperband"),
        project_name="student_habits_hb"
    )
    ...
    # 5) En iyi hiperparametreler ve model kaydet
    best_hp = tuner.get_best_hyperparameters(1)[0]
    print("🏆 Hyperband En İyi HP:")
    for key, val in best_hp.values.items():
        print(f"  {key}: {val}")

    best_model = tuner.get_best_models(1)[0]
    model_path = MODELS_DIR / "hb_tuned_ann.keras"
